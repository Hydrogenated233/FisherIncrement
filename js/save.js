...
var formatsave = {//+1
    ...
    startString: 'FisherIncSaveFormat',
...
function save() {//借鉴的
    addNotify("保存成功");
    localStorage.setItem("fisher-inc", formatsave.encode(player));
}
function load() {//+1
    addNotify("加载成功");
    let loadplayer = localStorage.getItem("fisher-inc");
    if (loadplayer != null) {
        let loadplayer = formatsave.decode(localStorage.getItem("fisher-inc"));
...
function loadText() {//+1
    ...
    if (loadplayer != null) {
        ;
        localStorage.setItem("fisher-inc", loadplayer)
        load();
    }
    addNotify("导入成功");
}
